...
babelConfigESM.presets[0][1].targets = {
  browsers: ["last 2 Chrome versions", "last 2 Safari versions", "last 2 iOS versions", "last 2 Firefox versions", "last 2 Edge versions"],
};
...
gulp.task("css-themes", () => gulp.src(["./css/theme/source/*.{sass,scss}"]).pipe(compileSass()).pipe(gulp.dest("./dist/theme")));
...
gulp.task("eslint", () => gulp.src(["./js/**", "gulpfile.js"]).pipe(eslint()).pipe(eslint.format()));
...
gulp.task(
  ...
  gulp.series(() =>
    gulp
      .src(["./index.html", "./dist/**", "./lib/**", "./images/**", "./plugin/**", "./**/*.md"], { base: "./" })
...
gulp.task("serve", () => {
  ...
  gulp.watch(["css/theme/source/*.{sass,scss}", "css/theme/template/*.{sass,scss}"], gulp.series("css-themes", "reload"));
